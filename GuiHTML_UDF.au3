...
Func _HTML_IECheckCompatible($vCheckMode = True)
	...
	Switch $_IE_Version
		...
		Case Else
			MsgBox(4096, 'Error', _
					'!!! Phiên bản Internet Explorer hiện tại trên máy bạn đã cũ (IE' & $_IE_Version & ').' & @CRLF & _
					'!!! Điều này khiến giao diện không thể hiển thị được.' & @CRLF & _
					'!!! Máy tính của bạn cần trình duyệt IE ở phiên bản cao hơn..')
			exit
	EndSwitch
	If $vCheckMode Then
